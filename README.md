...
1. 点「填入 1 段分镜」生成一个 segment（或后续用真实分集流程）。
2. 从左侧节点面板拖出「人物 / 场景 / 剧集内容 / 视频输出」节点。
3. 把资产节点和剧集内容节点连入「视频输出」节点——连线上的数字就是上下文顺序。
4. 选中「剧集内容」节点，绑定 segment、编辑镜头提示词。
5. 点「生成视频」——前端轮询任务状态，成功后内嵌播放生成的视频（mock 渠道为占位片）。
