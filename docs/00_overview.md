...
### ADR-005 · 资产归属单个项目，禁止跨项目复用

**决策**：视觉资产（人物 / 道具 / 场景）隶属于单个 `Project`。`Asset.project_id` 是资产访问、列表、删除与画布引用校验的边界；一个项目只能看到、选择、使用自己的资产。后端不提供全局资产列表，也不通过关联表把同一资产分享给多个项目。

**背景**：当前产品方向要求每个项目拥有独立资产，避免 A 项目的角色、道具或场景被 B 项目误用。项目页左侧承载分集与阶段导航，右侧展示当前阶段页面；资产管理是项目页内的一个阶段，而不是顶层全局入口。

**权衡**：放弃跨项目复用会带来少量重复资产，但换来更清晰的权限边界与更低的误引用风险。02→04/05 的出图产物直接写入当前项目资产表；如需复制到另一个项目，应显式创建另一条资产记录，而不是共享同一行。

**影响**：见 [01 领域模型](01_domain_model.md) 的 `Project 1:N Asset`，以及 [04 前端](04_frontend.md) 的项目页左侧导航 + 右侧资产页面流。
