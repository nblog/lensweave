...
关键算术（防"段数塌缩"，锁在 [06 §0.2](../test/instructions/06_分集分镜.md)）：
一个 180s 的 EP ⇒ `≥ ceil(180/15) = 12` 个 segment ⇒ 12 个分镜面板 ⇒ 12 段视频。3 集就是 ≥36 段，**不是 3 段**。这条约束直接决定了数据模型里 `Episode 1:N Segment` 的基数，是整个系统的脊椎。
...
**决策**：MVP = 项目 / 分集 CRUD + 全局资产库 + 画布 + **单 segment「图文生视频」端到端跑通**；8 阶段 agent 先只接 1–2 个关键阶段（06 分镜 + 08 视频装配），其余留接口桩。
